...
| PL-08 | Validazione e limiti sui file media | fatto | (vedi prossimo commit) | `uploadAudioFile` (js/firebase-init.js) ora rifiuta prima dell'upload i file non audio o oltre 8MB, con un `Error` dal messaggio chiaro. Non serve toccare js/render.js (a differenza di quanto ipotizzato dal piano): i due punti di chiamata (form domande, form effetti) avevano già un try/catch che mostra `e.message` nello stesso punto dove appare "Caricamento audio...", quindi ereditano il messaggio d'errore gratis. Nuovo test tests/e2e/phase1-media-validation.spec.js (tipo sbagliato + file sovradimensionato, su entrambi i punti di upload). Suite di regressione mirata (smoke, testmode-stats) verde. |
