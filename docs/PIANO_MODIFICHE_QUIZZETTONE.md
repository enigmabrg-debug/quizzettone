...
|---|---|---|---|---|---|
| FT-01 | Ambiente di prova separato | Creare un secondo progetto Firebase per sviluppo e collaudo, con configurazione distinta da quella di produzione. Eseguire prove, reset e migrazioni soltanto sul progetto di test. | Un reset di test non può modificare squadre, domande o partite reali. | Tutte le fasi successive, modalità prova, migrazione del modello dati. | RIEPILOGO, “Database di test e partita coincidenti” e Roadmap 1; BRAINSTORMING §12/QZ-15.<br><br>**Nota (deciso il 3 agosto 2026):** niente secondo progetto Firebase, sostituito con conferma rafforzata sul reset e checklist di backup manuale prima di ogni test (vedi nota operativa separata). |
| FT-02 | Regole Firebase conosciute e versionate | Controllare le regole effettive di Realtime Database e Storage, esportarle nel repository e documentare i percorsi leggibili e scrivibili. Per ora si mantiene l’accesso attuale; l’autorizzazione reale viene pianificata come seconda fase. | Le regole presenti in console coincidono con i file versionati; non ci sono dubbi su chi possa scrivere ciascun ramo. | Sicurezza dei dati, affidabilità degli errori, futura autenticazione. | AUDIT §2.2 e finding #2; RIEPILOGO “Regole Firebase e PIN admin”.<br><br>**Nota (regole verificate il 3 agosto 2026):** RTDB pubblica in lettura/scrittura, Storage aperta solo su `quizzettone-audio`. Link condiviso solo con gruppo ristretto → autenticazione confermata in Fase 2, non anticipata. Vedi `docs/REGOLE_FIREBASE_ATTUALI.md`. |
| FT-03 | Eliminazione XSS | Applicare escaping sistematico a nomi delle squadre e a qualsiasi altro testo proveniente dagli utenti prima di inserirlo nell’HTML. Preferire `textContent` quando non serve markup. | Nomi contenenti tag, virgolette o script vengono mostrati come testo innocuo in Team, Admin e Display. | Lobby, classifica, spettatori, albo d’oro. | AUDIT finding #1; RIEPILOGO “XSS nei nomi delle squadre”; BRAINSTORMING P0-01.<br><br>**Nota (implementato il 3 agosto 2026):** tutte le occorrenze di nome squadra in Team, Admin e Display ora passano da `escapeHtml()` (funzione già presente nel codice, usata in modo sistematico); l'unico campo di testo libero digitabile dagli utenti è il nome squadra al join. |
| FT-04 | Avvio e connessione comprensibili | Aggiungere stato iniziale di caricamento, timeout, messaggio di errore, indicatore online/offline e comando di retry. | Se il primo snapshot non arriva, nessuna vista resta bianca e l’utente sa come reagire. | Tutte le viste. | AUDIT finding #4; RIEPILOGO “Schermata bianca all’avvio”; BRAINSTORMING P0-03.<br><br>**Nota (implementato il 3 agosto 2026):** schermata di caricamento mostrata subito all'avvio, timeout di 8s (sia sul ripristino sessione sia sul primo snapshot) con schermata di errore e bottone "Riprova", indicatore online/offline riutilizzato anche in questa fase. Verificato con test automatizzati (Firebase reale bloccato in sandbox, sostituito da uno stub locale) su 7 scenari: rete che non risponde mai, errore immediato del listener, ripristino sessione appeso, vista Display appesa sul primo snapshot, oltre ai percorsi normali. |
| FT-05 | Errori Firebase gestiti | Fare restituire esiti espliciti alle operazioni `safeGet`, `safeSet` e `safeDelete`; mostrare “invio in corso”, successo, errore e retry nei flussi essenziali. Non avanzare l’interfaccia se una scrittura fallisce. | Join, risposta, cambio fase, correzione punti e reset non possono apparire riusciti se Firebase li ha rifiutati. | Lobby, risposte, regia, undo. | AUDIT §2.3 e finding #12; RIEPILOGO “Errori Firebase silenziosi”.<br><br>**Nota (implementato il 3 agosto 2026):** `safeGet`/`safeSet`/`safeDelete`/`safeList` restituiscono ora `{ok, value/error}` invece di valori ambigui (null/[]/niente). Join, risposta, cambio fase (Domanda successiva), correzione punti e reset mostrano invio in corso/errore e non avanzano l'interfaccia se Firebase rifiuta la scrittura (verificato con test automatizzato: scrittura di join bloccata → messaggio di errore, bottone riabilitato, utente resta sul form, non passa alla vista squadra). "Undo" non esiste ancora come funzione nel codice (è nel backlog, voce #43 della roadmap): nessun codice da instrumentare per ora. |
...
| FT-07 | Identità e presenza della squadra | Associare il dispositivo a un identificativo persistente, registrare `online`, ultimo contatto e riconnessione; usare `onDisconnect` senza cancellare squadra o punteggio. | Dopo refresh o perdita di rete la squadra recupera identità, domanda e stato; l’admin vede offline e ultimo contatto. | Lobby, ingresso tardivo, timer, conteggio delle risposte. | AUDIT finding #3; RIEPILOGO “Presenza online”; BRAINSTORMING §§5–6; REGOLAMENTO §14.2.<br><br>**Nota (implementato il 3 agosto 2026):** `teams/{id}` ha ora `online`/`lastSeenAt`; `onDisconnect()` registrato/ri-armato a ogni riconnessione (join, restore da URL/localStorage, ritorno online), aggiornamento parziale (non cancella id/name/joinedAt/punteggi). Uscita esplicita ("non sei tu?") annulla l'onDisconnect pendente e segna offline subito. Indicatore online/offline aggiunto in Lobby e nella tabella live squadre. L'identificativo persistente del dispositivo (`teamId` in URL+localStorage) esisteva già prima di questo intervento; qui si aggiunge lo stato di presenza sopra quella base. |
| FT-08 | Unicità e gestione dei nomi | Impedire la creazione concorrente di due squadre con lo stesso nome tramite indice normalizzato o transazione; consentire rinomina e risoluzione manuale da Admin. | Due join simultanei con lo stesso nome non generano squadre gemelle. | Lobby, albo d’oro, riconnessione. | AUDIT finding #6; BRAINSTORMING §5.<br><br>**Nota (implementato il 3 agosto 2026):** nuovo ramo `teamNames/{nomeNormalizzato}` (minuscolo, spazi superflui rimossi) → teamId, popolato via transazione Firebase: due join simultanei con lo stesso nome producono una sola squadra, il "perdente" rientra silenziosamente in quella creata dal "vincitore" (nessun errore, nessuna gemella). Decisione presa senza risposta esplicita alla domanda posta (usato il default raccomandato): digitare un nome già esistente fa sempre rientrare nella squadra esistente, sia in condizioni normali sia nella race — va confermata o corretta dall'utente. Rinomina/risoluzione manuale da Admin **non implementata** in questa sessione: non esiste ancora alcuna UI di gestione squadre lato Admin nel codice (è la voce futura #41 della roadmap, "Lobby con QR, Pronta e gestione squadre"); il criterio di completamento richiesto (niente squadre gemelle) è comunque soddisfatto dalla sola transazione. |
| FT-09 | Stato partita strutturato | Separare configurazione, stato operativo, squadre, istanze delle domande, risposte, punteggi ed eventi. Evitare di aggiungere altre chiavi a prefisso nello stesso nodo piatto. | Ogni fase legge e scrive un ramo chiaro; una nuova tipologia di domanda non richiede condizioni sparse in tutto il file. | Tutte le nuove meccaniche. | AUDIT §§2.1–2.2; BRAINSTORMING §§3–4 e dipendenze 95–99.<br><br>**Nota (implementato il 3 agosto 2026):** decisioni concordate — produzione considerata vuota/scartabile (nessuna migrazione necessaria); niente `sessions/{sessionId}` (una sola partita alla volta, fuori scope); risposte annidate per squadra come oggi (`answers/{teamId}/{qkey}`); niente `scoreLedger`/`events` nuovi, solo `overrides:<teamId>` rinominato in `scores/{teamId}` (correzioni manuali, stessa funzione di oggi). Rami creati: `config`, `runtime`, `questionInstances`, `teams/*`, `answers/*`, `scores/*`, `questionBank/*`, `soundEffects/*`; `scoringDefaults` invariato. Verificato con test comportamentale multi-pagina (admin + 2 squadre reali, server RTDB fittizio locale): join, risposta, correzione punti, cambio domanda, reset — tutti sul nuovo schema, nessun ramo piatto residuo. |
...
| FT-12 | Checkpoint non hardcoded | Eliminare il valore `8` dal testo e calcolare domande giocate e checkpoint dalla configurazione effettiva. | Qualunque numero di domande produce testi e passaggi coerenti. | Manche configurabili e preset. | AUDIT finding #8; RIEPILOGO “Numero 8 hardcoded”; BRAINSTORMING P0-06.<br><br>**Nota (implementato il 3 agosto 2026):** il testo del checkpoint di metà manche ora usa `Math.ceil(total/2)`, la stessa formula già usata in `adminNextQuestion` per far scattare il checkpoint — testo e trigger restano coerenti per qualunque numero di domande. |
